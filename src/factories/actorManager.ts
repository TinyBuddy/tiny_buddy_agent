...
export class ActorManager {
  ...
  // 根据类型创建Actor实例
  public async createActor(actorType: string, config?: ActorConfig): Promise<BaseActor> {
  ...
  public getActorsByType(actorType: string): BaseActor[] {
    ...
    return Array.from(this.actorInstances.values())
      .filter(actor => actor.type.toLowerCase() === typeLower);
  }
  ...
  public async destroyActor(actorId: string): Promise<boolean> {
    ...
    // 如果Actor有清理方法，调用它
    if (typeof (actor as any).cleanup === 'function') {
      await (actor as any).cleanup();
    }
