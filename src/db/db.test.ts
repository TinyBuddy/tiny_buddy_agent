...
async function runDbTests() {
	...
	try {
		...
		const storedWords =
			await vocabularyService.getVocabularyByChildId(TEST_CHILD_ID);
		...
		console.log(
			`数量验证: ${wordCount === TEST_WORDS.length ? "通过" : "失败"}`,
		);

		// 7. 测试词汇是否存在
		console.log("步骤7: 测试词汇是否存在");
		for (const word of TEST_WORDS) {
			const exists = await vocabularyService.isVocabularyExists(
				TEST_CHILD_ID,
				word,
			);
		...
		// 测试不存在的词汇
		const notExists = await vocabularyService.isVocabularyExists(
			TEST_CHILD_ID,
			"不存在的词汇",
		);
		...
		await vocabularyService.addVocabulary(
			TEST_CHILD_ID,
			TEST_WORDS.slice(0, 2),
		); // 只添加前两个词汇
		const updatedCount =
			await vocabularyService.getVocabularyCount(TEST_CHILD_ID);
		console.log(`添加重复词汇后数量: ${updatedCount}`);
		console.log(
			`重复添加测试: ${updatedCount === TEST_WORDS.length ? "通过" : "失败"}`,
		);
		...
		if (statsResult.length > 0) {
			console.log(
				`统计数据: 词汇数量=${statsResult[0].wordCount}, 最后更新=${statsResult[0].lastUpdated}`,
			);
			console.log(
				`统计数据验证: ${statsResult[0].wordCount === TEST_WORDS.length ? "通过" : "失败"}`,
			);
		} else {
