...
import type { Message, ConversationHistory } from '../models/message';
...
export interface MemoryService {
  ...
  updateChildProfile(childId: string, profile: Partial<ChildProfile>): Promise<ChildProfile>;
  ...
  trackLearningProgress(childId: string, knowledgePoint: string, progress: number): Promise<void>;
}
...
export class InMemoryMemoryService implements MemoryService {
  ...
  async updateChildProfile(childId: string, profile: Partial<ChildProfile>): Promise<ChildProfile> {
    ...
    // 获取或创建儿童档案
    let currentProfile = await this.getChildProfile(childId);
  ...
  async getConversationHistory(childId: string): Promise<Message[]> {
    ...
    return [...this.conversationHistories.get(childId)!.messages];
  }
  ...
  async analyzeChildInterests(childId: string): Promise<string[]> {
    ...
    const predefinedInterests: Record<string, string[]> = {
      '动物': ['猫', '狗', '兔子', '熊猫', 'bird', 'cat', 'dog', 'rabbit', 'panda'],
    ...
    for (const message of history) {
      if (message.type === 'user') {
        ...
        // 检查每个兴趣类别的关键词
        for (const [interest, keywords] of Object.entries(predefinedInterests)) {
          for (const keyword of keywords) {
            if (content.includes(keyword.toLowerCase())) {
              interestKeywords[interest] = (interestKeywords[interest] || 0) + 1;
            }
          }
        }
      }
    ...
    // 添加到已有兴趣中
    sortedInterests.forEach(interest => allInterests.add(interest));
  ...
  async trackLearningProgress(childId: string, knowledgePoint: string, progress: number): Promise<void> {
    ...
    await this.updateChildProfile(childId, { learningProgress: updatedProgress });
  }
