...
import type { NewVocabulary, NewVocabularyStats } from "../db/schema";
...
export class DatabaseVocabularyService implements VocabularyService {
	...
	async addVocabulary(childId: string, words: string[]): Promise<void> {
		...
		try {
			...
			console.log(
				`Added ${newWords.length} new vocabulary words for child ${childId}`,
			);
		} catch (error) {
	...
	async getVocabularyCount(childId: string): Promise<number> {
		try {
			...
			console.error(
				`Failed to get vocabulary count for child ${childId}:`,
				error,
			);
			return 0;
		}
	...
	async isVocabularyExists(childId: string, word: string): Promise<boolean> {
		try {
			const result = await db
				.select()
				.from(vocabulary)
				.where(and(eq(vocabulary.childId, childId), eq(vocabulary.word, word)))
				.limit(1);

			return result.length > 0;
		} catch (error) {
			console.error(
				`Failed to check vocabulary existence for child ${childId}:`,
				error,
			);
			return false;
		}
