...
function handleWebSocketConnection(ws: WebSocket, req: Request) {
  ...
  ws.on("close", (code, reason) => {
    console.log(
      `[${new Date().toISOString()}] WebSocket客户端连接关闭: ${code}, ${reason}`,
    );
    // 使用安全关闭函数关闭目标服务器连接
  ...
  targetWs.on("close", (code, reason) => {
    console.log(
      `[${new Date().toISOString()}] WebSocket目标服务器连接关闭: ${code}, ${reason}`,
    );
    // 使用安全关闭函数关闭客户端连接
...
wss.on("connection", (ws: WebSocket, req: Request) => {
  ...
  if (req.url === "/ws/proxy/hardware") {
    ...
    // 关闭不匹配的连接
    console.log(
      `[${new Date().toISOString()}] 拒绝WebSocket连接: 不支持的路径 ${req.url}`,
    );
    try {
...
app.get("/", (req: Request, res: Response) => {
  res.json({
    ...
    httpUsage:
      "POST/GET/PUT/PATCH/DELETE http://localhost:3000/api/proxy/{target-path}",
    httpExample:
...
server.listen(PORT, () => {
  ...
  console.log(
    `HTTP代理使用格式: http://localhost:${PORT}/api/proxy/{target-path}`,
  );
  console.log(
    `HTTP代理示例: http://localhost:${PORT}/api/proxy/api/devices/94a99031b91c/check-upgrade`,
  );
