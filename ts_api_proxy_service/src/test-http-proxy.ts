...
async function testHttpProxy() {
  try {
    ...
    try {
      ...
      console.log(
        "响应数据(前100字符):",
        JSON.stringify(getResponse.data).substring(0, 100) + "...",
      );
    } catch (getError) {
      console.log(
        "GET请求结果:",
        getError instanceof Error &&
          "response" in getError &&
          typeof (getError as any).response === "object" &&
          (getError as any).response !== null
    ...
    try {
      ...
      console.log(
        ...
        postError instanceof Error &&
          ...
          typeof (postError as any).response === "object" &&
          (postError as any).response !== null
    ...
    try {
      ...
      console.log(
        ...
        notFoundError instanceof Error &&
          "response" in notFoundError &&
          typeof (notFoundError as any).response === "object" &&
          (notFoundError as any).response !== null
